...
def metrics():
    ...
    col1.metric(label="Total Clients", value= df_selection.Gender.count(), delta="tout les clients")
    
    #col2.metric(label="Total salaire annuel", value= f"{df_selection.AnnualSalary.sum():,.0f}", delta=df.AnnualSalary.median()) 
    col2.metric(label="Total salaire annuel", value=f"{df_selection.AnnualSalary.sum():,.0f}", delta=df.AnnualSalary.median())

    
    col3.metric(label= "Salaire Annuel", value = f"{df_selection.AnnualSalary.max()-df.AnnualSalary.min():,.0f}", delta= "échelle Salariale annuelle") 
    
    style_metric_cards(background_color="#121270", border_left_color="#FFD700", border_color="#f20045", box_shadow="3px")
    
  

# créer les div pour les graphiques
...
def bar_chart():
    ...
    with div2:
        theme_plotly = None
        fig = px.bar(df_selection, x='Department', y='AnnualSalary', text_auto= '.2s', title='Salaires par Département')
